...
describe("first-party provider migration registry", () => {
  ...
  it("marks executable migrations as partial until oracle parity", () => {
    expect(FIRST_PARTY_PROVIDERS).toHaveLength(55);
    expect(PROVIDERS.filter((provider) => provider.status === "partial")).toHaveLength(55);
    expect(new Set(FIRST_PARTY_PROVIDERS.map((provider) => provider.id))).toHaveLength(55);
  });
