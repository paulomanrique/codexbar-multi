...
const definition: ProviderDefinition = {
  ...
  fetchUsage: async (ctx: ProviderContext) => {
    const base: any = (
      ctx.settings.get("DEEPGRAM_API_URL") || "https://api.deepgram.com/v1"
    ).replace(/\/+$/, "");

    function classifyStatus(status: any) {
      if (status === 401)
        return ctx.fail.authenticationExpired("Deepgram API key is invalid or expired.");
      if (status === 403) {
    ...
    async function getJSON(url: any) {
      ...
      try {
        ...
        throw ctx.fail.networkFailure(`Deepgram network error: ${message}`);
      }
    ...
    function optionalNumber(value: any, field: any, integer: any) {
    ...
    function optionalString(value: any, field: any) {
    ...
    function usagePayload(payload: any) {
      if (
        !payload ||
      ...
      if (payload.resolution !== null && payload.resolution !== undefined) {
        if (
          !payload.resolution ||
          typeof payload.resolution !== "object" ||
    ...
    const configuredProject: any = ctx.settings.get("DEEPGRAM_PROJECT_ID");
    let projects;
    if (configuredProject) {
      projects = [{ project_id: configuredProject, name: null }];
    } else {
      const payload: any = await getJSON(`${base}/projects`);
      ...
      projects = payload.projects.map((project: any, index: any) => {
        if (!project || typeof project !== "object" || typeof project.project_id !== "string") {
          throw ctx.fail.parseFailure(
            `Deepgram parse error: projects[${index}].project_id must be a string`,
          );
        }
        const name: any = optionalString(project.name, `projects[${index}].name`);
    ...
    for (const project of projects) {
      const payload: any = usagePayload(
    ...
    const decimal: any = (value: any) =>
      ctx.format.number(value, {
        minimumFractionDigits: value === Math.floor(value) ? 0 : 1,
        maximumFractionDigits: 1,
      });
    const integer: any = (value: any) => ctx.format.number(value, { maximumFractionDigits: 0 });
    const rows: any = [{ label: "Requests", value: integer(totals.requests) }];
...
export const deepgram: FirstPartyProvider = { ...strategy, descriptor };
