  ...
  fetchUsage: async (ctx: ProviderContext) => {
    ...
    try {
      ...
      throw ctx.fail.networkFailure(
        `ClinePass network error: ${_optionalChain([error, "optionalAccess", (_: any) => _.message]) || String(error)}`,
      );
    }
    ...
    try {
      ...
      throw ctx.fail.parseFailure(
        "Failed to parse ClinePass response: response was not valid JSON",
      );
    }
    ...
    if (payload.success !== true) {
      if (payload.success === false) {
        throw ctx.fail.parseFailure(
          "Failed to parse ClinePass response: Response success was false.",
        );
      }
    ...
    if (!Array.isArray(limits)) {
      throw ctx.fail.parseFailure(
        "Failed to parse ClinePass response: data.limits must be an array",
      );
    }
    ...
    for (const rawLimit of limits) {
      ...
      const limit: any = rawLimit;
      if (typeof limit.type !== "string") {
        throw ctx.fail.parseFailure(
          "Failed to parse ClinePass response: limit type must be a string",
        );
      }
      const minutes: any = windowMinutes[limit.type];
...
export const clinepass: FirstPartyProvider = { ...strategy, descriptor };
