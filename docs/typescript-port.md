...
- Node/platform adapters, native keyring credentials, normalized atomic config, SQLite WAL/migrations in a dedicated worker, a dedicated WAL reader outside the writer FIFO, bounded history/cost queries, persisted provider refresh, constant-size latest-snapshot overview reads, first CLI framework, Electron tray/popup shell, typed preload IPC, and React overview. Local scanner checkpoints and their rows commit atomically with compare-and-swap across desktop/CLI writers; `cost --refresh` now drives bounded Codex/Claude inventory, reports partial coverage, and owns signal cancellation. xAI daily vendor spend uses an atomic source-scoped ledger replacement and publishes only safe exact/estimated coverage metadata; Kiro enriches its CLI-authoritative report from the fixed `GetUsageLimits` API while retaining tokens inside the platform adapter. OpenCode Go declares ordered API/web strategies so explicit source selection and Auto fallback use the shared pipeline. Grok's bounded local session scanner is ported as a platform-owned primitive, and the desktop has a bounded Windows-alpha browser-session acquisition slice that starts at grok.com, limits navigation to Grok, xAI Accounts, and the explicitly declared Google, Apple, and X identity-provider origins, and stores only grok.com `sso`/`sso-rw` cookies behind typed IPC. The renderer now reloads a safe persisted-status projection for the default T3 Chat and Grok browser sessions, so those two buttons survive restart as connected, logged out, or fail-closed unavailable without exposing cookies or keyring keys. This status is only local credential-shape validation: it is not remote authentication verification, does not prove the upstream session is still accepted, and is not multi-account UI parity. The credential-backed browser-session broker can now read a selected Grok account key when the first-party runtime already resolved that selected token account, validates the resolved account ID before keyring reads, and does not fall back to the default browser session for selected reads; UI production of those selected browser-session records, full Grok browser parity, and broader multi-account browser parity remain pending. The opt-in Claude Swap desktop path lists safe account projections, reauthorizes opaque IDs from a fresh host listing, and keeps executable paths and helper output outside IPC.
