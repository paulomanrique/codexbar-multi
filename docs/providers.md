...
- CLI-based: runs `kiro-cli chat --no-interactive "/usage"` with a bounded timeout.
- Parses ANSI output for plan name, monthly credits percentage, bonus credits, and the CLI overage status.
- After the CLI refreshes its own credentials, a platform-owned read-only adapter calls the fixed AWS CodeWhisperer `GetUsageLimits` operation. It adds the overage-credit ceiling, accrued charges, currency, and reset without exposing the token to provider code.
- CLI output remains authoritative when private state or the enrichment request is unavailable; cancellation remains terminal.
